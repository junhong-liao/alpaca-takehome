...
export default function NotesPage() {
  ...
  async function handleRegenerate() {
    ...
    try {
      const response = await fetch(`/api/notes/${selectedNote.id}`, { method: "PATCH" });
      if (!response.ok) {
  ...
  async function handleNewNoteSubmit() {
    ...
    try {
      const res = await fetch("/api/notes", {
        ...
        body: JSON.stringify({ rawText: newRawText }),
      });
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <aside className="w-80 bg-white border-r flex flex-col">
        ...
        <button
          className="mx-4 mb-4 bg-blue-600 text-white text-center py-2 rounded hover:bg-blue-700 font-semibold"
        ...
        <div className="flex-1 overflow-y-auto">
          {notesLoading ? (
            ...
            <div className="p-4 text-gray-400">No notes yet. Create your first note!</div>
          ) : (
            <ul>
              {notes?.map((note: any) => (
                <li
                ...
                >
                  <div className="truncate text-sm text-gray-800">{note.generatedNote?.slice(0, 40) || note.rawText.slice(0, 40)}</div>
        ...
        {creating ? (
          <div className="w-full max-w-4xl">
            <div className="relative w-full bg-white rounded-md shadow-md min-h-[400px]">
              <div className="absolute top-4 left-4 text-3xl font-bold" style={{color:'#222C3A'}}>
                New Session Note
              </div>
              <textarea
                ref={textareaRef}
                className="w-full pt-16 px-4 pb-4 h-[400px] rounded-md outline-none text-black bg-white"
                value={newRawText}
          ...
          <div className="text-gray-500 text-xl" style={{color:'#222C3A'}}>Select a note to view</div>
        ) : (
          <div className="w-full max-w-4xl bg-white rounded shadow p-8">
            ...
            >
              {isEditingTitle ? (
                ...
                <h1 
                  className="text-3xl font-bold mb-4 cursor-text px-2 py-1 hover:bg-gray-100 rounded"
                  style={{color:'#222C3A'}}
                >
                  {noteTitle}
                </h1>
            ...
            <textarea
              className="w-full border rounded p-2 min-h-[120px] mb-2 text-black bg-gray-100"
              value={selectedNote.generatedNote || ""}
              readOnly
            />
